        ...
        <div class="bg-white rounded-lg shadow p-4">
            <div class="flex items-center justify-between mb-3">
                ...
                <div class="flex items-center gap-3 text-[10px]">
                    <span class="flex items-center gap-1"><span class="w-2 h-2 rounded-full bg-[#17C353]"></span> M</span>
                    <span class="flex items-center gap-1"><span class="w-2 h-2 rounded-full bg-[#FBED21]"></span> A</span>
                    <span class="flex items-center gap-1"><span class="w-2 h-2 rounded-full bg-[#EB2027]"></span> Unclear</span>
                </div>
            </div>
                ...
                <div class="flex-1 bg-gray-50 rounded flex items-end justify-around px-2">
                    ...
                    <div class="flex flex-col items-center gap-1 group" title="{{ $region }}">
                        <div class="flex gap-0.5 items-end">
                            <div class="w-3 bg-[#17C353] rounded-t transition-all hover:opacity-80" style="height: {{ min(($data['M'] / max(1, max(array_column($regionalStats, 'M')))) * 120, 120) }}px;" title="M: {{ number_format($data['M']) }}"></div>
                            <div class="w-3 bg-[#FBED21] rounded-t transition-all hover:opacity-80" style="height: {{ min(($data['A'] / max(1, max(array_column($regionalStats, 'A')))) * 120, 120) }}px;" title="A: {{ number_format($data['A']) }}"></div>
                            <div class="w-3 bg-[#EB2027] rounded-t transition-all hover:opacity-80" style="height: {{ min(($data['unclear'] / max(1, max(array_column($regionalStats, 'unclear')))) * 120, 120) }}px;" title="Unclear: {{ number_format($data['unclear']) }}"></div>
                        </div>
                        <span class="text-[9px] text-gray-600 truncate max-w-[50px]">{{ explode(' ', $region)[0] }}</span>
                    </div>
                    @endforeach
                </div>
            </div>
            ...
            <div class="flex gap-4">
                <div class="flex-1 space-y-1.5">
                    ...
                    <div class="flex items-center gap-2 text-sm group cursor-pointer hover:bg-gray-50 py-0.5 px-1 rounded" 
                        ...
                        <span class="w-2 h-2 rounded-full flex-shrink-0" style="background-color: {{ $data['color'] }};"></span>
