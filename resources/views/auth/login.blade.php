...
<body class="min-h-screen bg-gradient-to-br from-[#E8F4FC] to-[#D1E9F6] flex items-center justify-center p-4"
    ...
    <div class="bg-white rounded-2xl shadow-xl overflow-hidden w-full max-w-md">
        ...
        <div class="relative h-28 flex items-end justify-center pb-2"
            style="background: radial-gradient(ellipse at top center, #29AAE1 0%, rgba(41, 170, 225, 0.3) 40%, transparent 70%);">
            ...
            <form method="POST" action="{{ route('login.submit') }}" class="space-y-4">
                ...
                <div>
                    ...
                    <input type="email" name="email" value="{{ old('email') }}" placeholder="Enter your email"
                        class="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#29AAE1] text-sm"
                        required>
                </div>
                ...
                <div>
                    ...
                    <div class="relative" x-data="{ showPassword: false }">
                        ...
                        <button type="button" @click="showPassword = !showPassword"
                            ...
                            <svg :class="showPassword ? 'hidden' : 'block'" class="w-5 h-5" fill="none"
                            ...
                            <!-- Eye closed icon (show when password is visible) -->
                            <svg :class="showPassword ? 'block' : 'hidden'" class="w-5 h-5" fill="none"
                                stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                ...
                <button type="submit"
                    class="w-full bg-[#29AAE1] hover:bg-[#1E8CC0] text-white font-semibold py-2.5 rounded-lg transition-colors">
                    Sign In
                </button>
            </form>
        </div>
