...
@section('content')
    <div x-data="userManagement()" x-init="init()" class="space-y-6">
        ...
        <div x-show="showToast" x-cloak x-transition:enter="transition ease-out duration-300"
            ...
            x-transition:leave="transition ease-in duration-200" class="fixed top-4 right-4 z-[60]">
            <div :class="toastType === 'success' ? 'bg-[#17C353]' : 'bg-[#EB2027]'"
                ...
                <div class="w-8 h-8 bg-white rounded-full flex items-center justify-center flex-shrink-0">
                    <svg x-show="toastType === 'success'" class="w-5 h-5 text-[#17C353]" fill="none" stroke="currentColor"
                        viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7" />
                    </svg>
                    <svg x-show="toastType === 'error'" class="w-5 h-5 text-[#EB2027]" fill="none" stroke="currentColor"
                        viewBox="0 0 24 24">
        ...
        <div class="flex items-center justify-between">
            ...
            <div class="flex items-center gap-3">
                ...
                <button @click="openAddModal()"
                    class="flex items-center gap-2 px-4 py-2 border-2 border-[#29AAE1] text-[#29AAE1] rounded-lg hover:bg-[#29AAE1] hover:text-white transition-colors">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div class="flex flex-wrap items-center gap-4">
            ...
            <div>
                ...
                <select x-model="roleFilter" @change="applyFilters()"
                    class="px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#29AAE1]">
                ...
                <tbody class="divide-y divide-gray-200">
                    @forelse($users as $index => $user)
                        <tr class="hover:bg-gray-50">
                            ...
                            <td class="px-4 py-3">
                                <span
                                    class="px-3 py-1 rounded-full text-xs font-medium {{ $user->status === 'active' ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700' }}">
                                    {{ ucfirst($user->status) }}
                                </span>
                            </td>
                            ...
                            <td class="px-4 py-3">
                                <div class="flex items-center gap-2">
                                    ...
                                    <button @click="openEditModal({{ json_encode($user) }})"
                                        class="p-1 text-gray-500 hover:text-[#FBED21]" title="Edit">
                                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                    ...
                                    <button
                                        @click="openDeleteModal('{{ $user->id }}', '{{ $user->name }}', '{{ $user->email }}')"
                                        class="p-1 text-gray-500 hover:text-[#EB2027]" title="Delete">
                                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
        ...
        <div x-show="showModal" x-cloak x-transition.opacity
            class="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4" @click.self="showModal = false">
            <div x-show="showModal" x-transition class="bg-white rounded-lg shadow-xl w-full max-w-md">
                ...
                <form :action="editingUser ? '/users/' + editingUser.id : '{{ route('users.store') }}'" method="POST"
                    class="p-4 space-y-4">
                    @csrf
                    ...
                    <div>
                        ...
                        <input type="text" name="name" x-model="formData.name" placeholder="Enter name"
                            class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#29AAE1]"
                            required>
                    </div>
                    ...
                    <div class="flex justify-end gap-3 pt-4">
                        ...
                        <button type="submit" class="px-6 py-2 bg-[#29AAE1] text-white rounded-lg hover:bg-[#1E8CC0]"
                            x-text="editingUser ? 'Update User' : 'Add User'"></button>
                    </div>
                </form>
            </div>
        </div>
        ...
        <div x-show="showDeleteModal" x-cloak x-transition.opacity
            ...
            <div x-show="showDeleteModal" x-transition
                ...
                <div
                    class="bg-red-50 border border-red-200 rounded-lg px-4 py-2 mb-6 flex items-center gap-2 justify-center">
                ...
                <div class="flex gap-3">
                    ...
                    <form :action="'/users/' + deleteUserId" method="POST" class="flex-1">
                        ...
                        @method('DELETE')
                        <button type="submit"
                            class="w-full px-4 py-2 bg-[#EB2027] text-white rounded-lg hover:bg-red-700">Yes,
                            Delete</button>
                    </form>
                </div>
            </div>
            ...
            <div x-show="showLogoutModal" x-transition
                ...
                <div class="flex gap-3">
                    ...
                    <form action="{{ route('logout') }}" method="POST" class="flex-1">
                        @csrf
                        <button type="submit"
                            class="w-full px-4 py-2 bg-[#EB2027] text-white rounded-lg hover:bg-red-700">Yes, Log
                            Out</button>
                    </form>
                </div>
            </div>
