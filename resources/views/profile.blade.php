...
@section('content')
    ...
    <div x-data="profilePage()" x-init="init()" class="min-h-[calc(100vh-120px)] py-6 px-4 sm:px-6 lg:px-8">
        ...
        <template x-if="showCropperModal">
            <div class="crop-overlay" @mousedown.self="startDrag($event)" @mousemove="onDrag($event)" @mouseup="endDrag()"
                @mouseleave="endDrag()" @touchstart.self="startDrag($event)" @touchmove="onDrag($event)"
                @touchend="endDrag()" @wheel.prevent="onZoom($event)">

                <!-- Header -->
                <div
                    class="absolute top-0 left-0 right-0 z-10 flex items-center justify-between px-4 py-4 bg-gradient-to-b from-black/50 to-transparent">
                    <button @click="closeCropper()"
                        class="text-white hover:text-gray-300 p-2 rounded-full hover:bg-white/10 transition-colors">
                        <svg class="w-7 h-7" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                d="M6 18L18 6M6 6l12 12" />
                        </svg>
                    </button>
                    <span class="text-white font-medium text-lg">Drag the image to adjust</span>
                    <span class="w-11"></span>
                </div>

                <!-- Image Container -->
                <div class="crop-image-container" id="cropImageContainer" @mousedown="startDrag($event)"
                    @touchstart="startDrag($event)">
                    <img id="cropperImage" :src="originalImageSrc" alt="Crop preview"
                        :style="'transform: translate(' + cropTranslateX + 'px, ' + cropTranslateY + 'px) scale(' + cropScale + '); transition: ' + (isDragging ? 'none' : 'transform 0.15s ease-out')">
                </div>
                ...
                <div class="absolute right-4 top-1/2 -translate-y-1/2 z-10 flex flex-col gap-3">
                    <button @click="zoomIn()"
                        class="w-12 h-12 bg-white/20 backdrop-blur-md rounded-xl flex items-center justify-center text-white hover:bg-white/30 transition-all active:scale-95">
                ...
                <button @click="applyCrop()"
                    class="absolute bottom-8 right-8 z-10 w-16 h-16 bg-[#29AAE1] rounded-full flex items-center justify-center shadow-2xl hover:bg-[#1E8CC0] transition-all hover:scale-110 active:scale-95">
            ...
            function profilePage() {
                return {
                    ...
                    updateCircleSize() {
                        ...
                        this.circleSize = Math.min(320, minDimension * 0.65);
                    },
                    ...
                    handleAvatarChange(event) {
                        ...
                        reader.onload = (e) => {
                            ...
                            img.onload = () => {
                                ...
                                this.cropScale = (this.circleSize / minImageDim) * 1.3;
                    ...
                    onZoom(event) {
                        ...
                        this.cropScale = Math.max(0.3, Math.min(5, this.cropScale + delta));
                    },
                    ...
                    zoomOut() {
                        this.cropScale = Math.max(0.3, this.cropScale - 0.2);
                    },
                    ...
                    applyCrop() {
                        ...
                        if (!img) {
                            this.showErrorToast('Image not loaded');
                            return;
                        }
                        ...
                        const displayWidth = img.naturalWidth * this.cropScale;
                        const displayHeight = img.naturalHeight * this.cropScale;

                        // Image center position (center of screen + translation)
                        const imgCenterX = containerWidth / 2 + this.cropTranslateX;
                        const imgCenterY = containerHeight / 2 + this.cropTranslateY;

                        // Image top-left corner
                        const imgLeft = imgCenterX - displayWidth / 2;
                        const imgTop = imgCenterY - displayHeight / 2;

                        // Circle center (always center of screen)
                        ...
                        canvas.toBlob((blob) => {
                            if (blob) {
                                ...
                                this.closeCropper();
                                this.showSuccessToast('Image cropped! Click Save Changes to apply.');
                            }
                        }, 'image/jpeg', 0.92);
                    },
