...
<div x-data="mapPage()" x-init="init()" class="relative h-[calc(100vh-56px)] flex">
    ...
    <div class="flex-1 relative">
        ...
        <div class="absolute top-4 left-1/2 -translate-x-1/2 z-[1000] flex items-center gap-4">
            ...
            <div class="relative" x-data="{ open: false }">
                <button @click="open = !open" class="bg-white rounded-lg shadow-lg px-5 py-2.5 flex items-center gap-2 text-sm text-gray-700 hover:bg-gray-50 min-w-[140px]">
                    Regional
            ...
            <div class="relative" x-data="{ open: false }">
                <button @click="open = !open" class="bg-white rounded-lg shadow-lg px-5 py-2.5 flex items-center gap-2 text-sm text-gray-700 hover:bg-gray-50 min-w-[120px]">
                    Status
                    <svg class="w-4 h-4 ml-auto" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"/>
                    </svg>
            ...
            <div x-show="selectedPoint" class="bg-white rounded-lg shadow-lg px-4 py-2.5 text-sm font-medium text-gray-700">
        ...
        <div x-show="hoveredPoint" x-transition
             class="fixed bg-white rounded-xl shadow-2xl p-4 z-[1002] min-w-[200px]"
             :style="'left:' + popupX + 'px; top:' + popupY + 'px'">
            <div class="flex items-center gap-2 mb-2">
                <div class="w-5 h-5 border-2 rounded-full"
                     :class="hoveredPoint?.kdam === 'M' ? 'border-[#17C353]' : hoveredPoint?.kdam === 'A' ? 'border-[#FBED21]' : 'border-[#EB2027]'"></div>
            ...
            <div class="flex justify-between text-sm">
                ...
                <div class="text-right">
                    ...
                    <p class="font-bold" :class="hoveredPoint?.kdam === 'M' ? 'text-[#17C353]' : hoveredPoint?.kdam === 'A' ? 'text-[#FBED21]' : 'text-[#EB2027]'"
                       x-text="hoveredPoint?.kdam === 'M' ? 'Meterisasi' : hoveredPoint?.kdam === 'A' ? 'Abonemen' : 'Unclear'"></p>
                </div>
            </div>
        </div>
    </div>
    
    <!-- Detail Panel (Right Side) - Original Design -->
    <div x-show="selectedPoint" x-transition:enter="transition ease-out duration-300" 
        ...
        <!-- Photo Section with PLN watermark -->
        <div class="relative h-48 bg-gray-200">
            <img src="{{ asset('images/pln-sipju-logo.png') }}" class="absolute inset-0 w-full h-full object-contain p-8 opacity-20">
            <div class="absolute bottom-2 right-2 bg-yellow-400 text-xs font-bold px-2 py-1 rounded">
                PJU
            </div>
        </div>
        ...
        <div class="flex-1 overflow-y-auto p-4 space-y-3 text-sm">
            <div class="flex justify-between border-b pb-2">
                <span class="text-gray-500">No ID Pel</span>
                <span class="font-semibold text-gray-900">:</span>
                <span class="font-medium text-gray-800" x-text="selectedPoint?.idpel || '-'"></span>
            </div>
            <div class="flex justify-between border-b pb-2">
                <span class="text-gray-500">Nama</span>
                <span class="font-semibold text-gray-900">:</span>
                <span class="font-medium text-gray-800" x-text="selectedPoint?.nama || '-'"></span>
            </div>
            <div class="flex justify-between border-b pb-2">
                <span class="text-gray-500">Tarif / Daya</span>
                <span class="font-semibold text-gray-900">:</span>
                <span class="font-medium text-[#29AAE1]" x-text="(selectedPoint?.tarif || '-') + ' / ' + (selectedPoint?.daya || '-') + ' VA'"></span>
            </div>
            <div class="flex justify-between border-b pb-2">
                <span class="text-gray-500">Wilayah Dishub</span>
                <span class="font-semibold text-gray-900">:</span>
                <span class="font-medium text-[#29AAE1]" x-text="selectedPoint?.nama_kabupaten?.replace('KAB. ', '').replace('KOTA ', '') || '-'"></span>
            </div>
            <div class="flex justify-between border-b pb-2">
                <span class="text-gray-500">Alamat</span>
                <span class="font-semibold text-gray-900">:</span>
                <span class="font-medium text-[#29AAE1]" x-text="selectedPoint?.alamat || '-'"></span>
            </div>
            <div class="flex justify-between border-b pb-2">
                <span class="text-gray-500">Status Meter</span>
                <span class="font-semibold text-gray-900">:</span>
                <span class="font-medium" :class="selectedPoint?.kdam === 'M' ? 'text-[#17C353]' : selectedPoint?.kdam === 'A' ? 'text-[#FBED21]' : 'text-[#EB2027]'"
                      x-text="selectedPoint?.kdam === 'M' ? 'Meterisasi' : selectedPoint?.kdam === 'A' ? 'Abonemen' : 'Unclear'"></span>
            </div>
            <div class="flex justify-between border-b pb-2">
                <span class="text-gray-500">No Meter</span>
                <span class="font-semibold text-gray-900">:</span>
                <span class="font-medium text-[#29AAE1]" x-text="selectedPoint?.no_meter ? 'PRABAYAR (' + selectedPoint.no_meter + ')' : '-'"></span>
            </div>
            <div class="flex justify-between border-b pb-2">
                <span class="text-gray-500">Jumlah Lampu</span>
                <span class="font-semibold text-gray-900">:</span>
                <span class="font-medium text-[#EB2027] font-bold" x-text="selectedPoint?.jumlah_lampu || 1"></span>
            </div>
            <div class="flex justify-between border-b pb-2">
                <span class="text-gray-500">Data Gardu</span>
                <span class="font-semibold text-gray-900">:</span>
                <span class="font-medium text-[#29AAE1]" x-text="selectedPoint?.gardu || '-'"></span>
            </div>
            <div class="border-b pb-2">
                <span class="text-gray-500">Titik Koordinat :</span>
                <p class="font-mono text-xs text-gray-700 mt-1" x-text="(selectedPoint?.koordinat_x || '-') + ', ' + (selectedPoint?.koordinat_y || '-')"></p>
            </div>
        </div>
    </div>
</div>
...
function mapPage() {
    return {
        ...
        addMarker(point) {
            ...
            marker.on('mouseout', () => {
                if (this.selectedPoint?.idpel !== this.hoveredPoint?.idpel) {
